...
# you want <%= bus_name.last %> to provide
<% indent, open_code, close_code = ::Roby::CLI::Gen.in_module(*bus_name[0..-2]) %>
<%= open_code %>
<%= indent %>com_bus_type '<%= bus_name.last %>', message_type: '/int' do
...
<%= indent %>    # # robot do
<%= indent %>    # #     com_bus <%= bus_name.last %>, as: 'bus' do
<%= indent %>    # #         device(<%= bus_name.last %>SpecificDevice, as: 'dev').
...
<%= indent %>    # #     driver_for <%= bus_name.last %>, as: 'driver'
