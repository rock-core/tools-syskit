    ...
    module RobyPlugin
	    ...
	    def remove_all
            ...
            def apply_merge_to_stored_instances
                ...
                mappings = Hash.new
                instances.each do |instance|
                    new_task = (mappings[instance.task] ||= replacement_for(instance.task))
                ...
                robot.devices.each do |name, dev|
                    ...
                    new_task = (mappings[dev.task] ||= replacement_for(dev.task))
                ...
                @tasks = tasks.map_value do |name, task|
                    new_task = (mappings[task] ||= replacement_for(task.to_component))
                    if new_task != task
                    ...
                    if new_task.respond_to?(:child_selection)
                        new_task.child_selection.each_value do |instance_selection|
                            if child_task = instance_selection.selected_task
                                mapped_child_task = (mappings[child_task] ||= replacement_for(child_task.to_component))
                ...
                if @deployment_tasks
                    @deployment_tasks = @deployment_tasks.map do |task|
                        new_task = (mappings[task] ||= replacement_for(task))
                        if new_task != task
