...
module Syskit
    module ProcessManagers
        # The in-process process manager allows to instanciate orogen-created tasks
        # inside the Ruby process itself
        #
        # @see Configuration#use_in_process_tasks DeploymentGroup#use_in_process_tasks
        module InProcess
            # Manager of in-process deployments
            class Manager
                ...
                def start(
                    name, deployment_name = name, name_mappings = {}, prefix: nil, **_
                )
                    if processes[name]
                ...
                def self.default_logger_task(plan, app: Roby.app)
                    ...
                    plan.add_permanent_task(
                        logger_t = deployment_t.task(DEFAULT_LOGGER_NAME)
                    )
                    if logger_t.respond_to?(:default_logger=)
                        logger_t.default_logger = true
                    end
                    logger_t
                end
