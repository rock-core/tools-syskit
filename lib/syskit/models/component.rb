            ...
            def can_merge_service?(self_srv, target_srv)
                if target_srv.model != self_srv.model
                    NetworkGeneration::MergeSolver.debug do
                        "rejecting #{self}.merge(#{target_model}): dynamic "\
                        "service #{self_srv.name} is of model "\
                        "#{self_srv.model.short_name} on #{self} and of "\
                        "model #{target_srv.model.short_name} on #{target_model}"
                    end
                    false
                    ...
                    NetworkGeneration::MergeSolver.debug do
                        "rejecting #{self}.merge(#{target_model}): "\
                        "#{self_srv.name} is a dynamic service on the receiver, "\
                        "but a static one on the target"
                    end
                    false
                else
                    true
