...
module Syskit
    describe "Abstract placeholders" do
        describe "Fullfilled models" do
...
class TC_AbstractPlaceholders < Minitest::Test
    ...
    def test_proxy_data_services
		...
		assert_equal(([Syskit::AbstractComponent, Syskit::DataService] + services).to_set, proxy.fullfilled_model.to_set)
    ...
    def test_proxy_task_and_data_service_mix
		...
		assert_equal(services.to_set, proxy.proxied_data_services.to_set)
        assert_equal(([task_model, Syskit::AbstractComponent, Syskit::Component,
            Roby::Task, Syskit::DataService] + services).to_set,
            proxy.fullfilled_model.to_set)
    ...
    def test_each_fullfilled_model_yields_real_task_model_as_well_as_proxied_services
        ...
        proxy = Syskit.proxy_task_model_for(services + [task_model])
        assert_equal [task_model, Syskit::AbstractComponent,
            Syskit::TaskContext, Syskit::Component, Roby::Task,
            Syskit::DataService, *services].to_set,
            proxy.each_fullfilled_model.to_set
    end
