...
class TC_RobySpec_DataServiceModels < Test::Unit::TestCase
    ...
    def test_model_declaration_requires_constant_name
        assert_raises(ArgumentError) { sys_model.data_service_type("image") }
    end

    def test_model_placeholder_task_model
        model = sys_model.data_service_type("Image")
        fake_task_model = mock_roby_task_context_model
        fake_task_model.provides model
        flexmock(Orocos::RobyPlugin).should_receive(:placeholder_model_for).
            once.
            with(any, [model]).
            and_return(fake_task_model)

        placeholder_model = model.task_model
    ...
    def test_provides_can_override_port_using_port_mappings
        parent_model = sys_model.data_service_type("Base") do
            output_port "out", "/int32_t"
        end

        model = sys_model.data_service_type("Submodel") do
            output_port "out", "/double"
