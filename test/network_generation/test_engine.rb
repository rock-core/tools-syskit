...
describe Syskit::NetworkGeneration::Engine do
    ...
    describe "#compute_deployed_models" do
        it "should register all fullfilled models for deployed tasks" do
            ...
            assert_equal provided_models.to_value_set, orocos_engine.compute_deployed_models.to_value_set
        end
        it "should be able to discover compositions that are enabled because of deployed tasks" do
            ...
            assert_equal [service_model, task_model, composition_model].to_value_set,
                orocos_engine.compute_deployed_models.to_value_set
        end
    ...
    describe "#compute_task_context_deployment_candidates" do
        it "lists the deployments on a per-model basis" do
            ...
            result = orocos_engine.compute_task_context_deployment_candidates
    ...
    describe "#resolve_deployment_ambiguity" do
        it "resolves ambiguity by orocos_name" do
            ...
            assert_equal candidates[1],
                orocos_engine.resolve_deployment_ambiguity(candidates, flexmock(:orocos_name => 'other_task'))
        end
        it "resolves ambiguity by deployment hints if there are no name" do
            ...
            assert_equal candidates[1],
                orocos_engine.resolve_deployment_ambiguity(candidates, task)
        end
        it "returns nil if there are neither an orocos name nor hints" do
            candidates = [['localhost', Object.new, 'task'], ['other_machine', Object.new, 'other_task']]
            requirements = flexmock(:deployment_hints => [])
            task = flexmock(:orocos_name => nil, :requirements => requirements)
            assert !orocos_engine.resolve_deployment_ambiguity(candidates, task)
        end
    ...
    describe "#deploy_system_network" do
        ...
        before do
            ...
            flexmock(orocos_engine).should_receive(:compute_task_context_deployment_candidates).
                and_return(deployments).by_default
            orocos_engine.prepare(:validate_network => false)
            flexmock(orocos_engine.work_plan).should_receive(:find_local_tasks).
        ...
        it "creates the necessary deployment task and uses #task to get the deployed task context" do
            ...
            flexmock(orocos_engine.work_plan).should_receive(:find_local_tasks).and_return([task])
            ...
            flexmock(orocos_engine.work_plan).should_receive(:add).once.with(deployment_task).ordered
            # Create the task
            ...
            flexmock(orocos_engine.merge_solver).should_receive(:merge).once.with(task, deployed_task)
            orocos_engine.deploy_system_network
        end
        it "instanciates the same deployment only once on the same machine" do
            ...
            flexmock(orocos_engine.work_plan).should_receive(:add)
            flexmock(orocos_engine.merge_solver).should_receive(:merge)

            task0 = task_models[0].new(:orocos_name => 'task')
            task1 = task_models[0].new(:orocos_name => 'other_task')

            flexmock(orocos_engine.work_plan).should_receive(:find_local_tasks).and_return([task0, task1])
            # Create on the right host
            flexmock(deployment_models[0]).should_receive(:new).once.
                with(:on => 'machine').
                and_return(deployment_task = flexmock)
            ...
            # And finally replace the task with the deployed task
            assert_equal [], orocos_engine.deploy_system_network
        end
        ...
        it "does not allocate the same task twice" do
            ...
            flexmock(deployment_models[0]).should_receive(:new).once.
                and_return(deployment_task0 = flexmock)
            deployment_task0.should_receive(:task).with('task').once
            assert_equal [task1], orocos_engine.deploy_system_network.to_a
        end
        ...
        it "does not consider already deployed tasks" do
            ...
            flexmock(orocos_engine).should_receive(:compute_task_context_deployment_candidates).
                and_return(deployments)
            flexmock(orocos_engine.work_plan).should_receive(:add).never
            flexmock(orocos_engine.merge_solver).should_receive(:merge).never
    ...
    describe "#adapt_existing_deployment" do
        ...
        before do
            ...
            orocos_engine.work_plan.add(task)
            orocos_engine.real_plan.add(existing_task)
            @existing_task = orocos_engine.work_plan[existing_task]
            @existing_deployment_task = orocos_engine.work_plan[existing_deployment_task]
        end

        def should_not_create_new_task
            flexmock(existing_deployment_task).should_receive(:task).never
            flexmock(orocos_engine.merge_solver).should_receive(:merge).once.
        ...
        it "creates a new deployed task if there is not one already" do
            ...
            orocos_engine.adapt_existing_deployment(deployment_task, existing_deployment_task)
        end
