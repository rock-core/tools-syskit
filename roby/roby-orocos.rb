...
module Roby
    module Orocos
        ...
        # task context will be registered as Roby::Orocos::Imu::Driver
        ...
        def self.orogen_project_module(name)
            ...
            Roby::Orocos.define_or_reuse(const_name) do
                mod = Project.new
        ...
        class Deployment < Roby::Task
            ...
            # deployment.
            def task(name)
                full_name = "#{self.class.deployment_name}_#{name}" 
                activity = orogen_spec.task_activities.find { |act| name == act.name || full_name == act.name }
                if !activity
            ...
            event :stop do
                ...
                # Add an intermediate event that 
            ...
            def self.define_from_orogen(deployment_spec)
                ...
                Roby::Orocos::Deployments.const_set(deployment_spec.name.camelcase(true), klass)
                klass
            end
        end
        ...
        def self.update(plan) # :nodoc:
            if !(query = plan.instance_variable_get :@orocos_update_query)
                query = plan.find_tasks(Roby::Orocos::TaskContext)
        ...
        class TaskContext < Roby::Task
            ...
            event :start do
                ...
                actual_task = deployment.orogen_deployment.orogen.task_activities.find do |task|
                    task.context.name == orogen_spec.name
                end

                @orogen_task = ::Orocos::TaskContext.get(actual_task.name)
            ...
            def self.define_from_orogen(task_spec)
                klass = Class.new(TaskContext)
                klass.instance_variable_set :@orogen_spec, task_spec
                namespace = Roby::Orocos.orogen_project_module(task_spec.component.name)
                ...
                if task_spec.extended_state_support?
                    ...
                    task_spec.states.each_with_index do |name, type|
